...
def test_undirected_graph():
    ...
    wi_neighbors = [[[int(n) for n in wi.neighbors(graph, node)] for node in graph.get_nodes()] for graph in wi_graphs]
    wi_boundaries = [[int(n) for n in wi.boundary(graph, np.array(graph.get_nodes()[:5]))] for graph in wi_graphs]
...
def test_directed_graph():
    ...
    wi_parents = [[sorted([int(n) for n in wi.parents(graph, node)]) for node in graph.get_nodes()] for graph in wi_graphs]
    wi_children = [[sorted([int(n) for n in wi.children(graph, node)]) for node in graph.get_nodes()] for graph in wi_graphs]
    wi_ancestors = [[sorted([int(n) for n in wi.ancestors(graph, node)]) for node in graph.get_nodes()[:5]] for graph in wi_graphs]
    wi_descendants = [[sorted([int(n) for n in wi.descendants(graph, node)]) for node in graph.get_nodes()[:5]] for graph in wi_graphs[:3]]
